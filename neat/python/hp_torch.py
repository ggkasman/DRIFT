...
def fit_func(params, data_path, experiment_id, args, fast):
    ...
    neat_model = nn.Sequential(
        params["net_x_arch_trunk"],
        params["net_y_size_trunk"],
        params["mu_top_layer"],
        params["sd_top_layer"],
        params["top_layer"],
    )
...
'''The evaluate function was added to handle model evaluation in PyTorch because PyTorch does not have a built-in method for model evaluation like Keras' model.evaluate()'''
def evaluate(model, data_loader):
    ...
    with torch.no_grad():
        for x, y in data_loader:
            y_pred = model(x)
            total_log_lik += -0.5 * ((y - y_pred) ** 2).sum().item()
    return total_log_lik / len(data_loader.dataset)
