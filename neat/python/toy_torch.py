...
def run_tp(X, y):
    mod = get_neat_model(
        dim_features=5,
        net_x_arch_trunk=relu_network((5, 100,100), dropout=0),
        net_y_size_trunk=nonneg_tanh_network([50, 50, 32], dropout=0),
    ...
    early_stopping = EarlyStopping(patience=50, monitor="val_logLik", restore_best_weights=False)
...
def run_inter(X, y):
    mod = get_neat_model(
        ...
        net_x_arch_trunk=relu_network((5,0), dropout=0),
        net_y_size_trunk=nonneg_tanh_network((50,32), dropout=0),
        base_distribution=torch.distributions.Normal(0, 1),
        optimizer=optim.Adam,
        model_type=ModelType.INTER,
        top_layer=layer_nonneg_lin(32,1),
    )
    print(mod)
...
def run_ls(X, y):
    ...
    mod = get_neat_model(
        ...
        sd_top_layer=layer_inverse_exp(100,units=1),
        top_layer=layer_nonneg_lin(32,1),
    )
    
    
    early_stopping = EarlyStopping(patience=200, monitor="val_logLik", restore_best_weights=True)
    ...
    for epoch in range(55):
        ...
        for X_batch, y_batch in train_loader:
            ...
            print(f"Input X shape: {y_batch.shape}")
            optimizer.zero_grad()
            outputs = mod(X_batch, y_batch)
            loss = criterion(outputs, y_batch)
            loss.backward()
...
def get_toy_data():
    ...
    X = np.loadtxt("/home/pankhil/AppDL/DRIFT/neat/tests/toy_data_X.csv", delimiter=",")
    y = np.loadtxt("/home/pankhil/AppDL/DRIFT/neat/tests/toy_data_y.csv", delimiter=",").reshape(-1, 1)
    return X, y
